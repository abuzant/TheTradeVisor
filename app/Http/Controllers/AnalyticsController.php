...
class AnalyticsController extends Controller
{
    ...
    private function getOverviewStats($days = 30)
    {
        return [
            ...
            'total_volume' => Deal::closedTrades()
                                  ->dateRange(now()->subDays($days))
                                  ->sum('volume'),
            'total_trades' => Deal::closedTrades()
                                  ->dateRange(now()->subDays($days))
                                  ->count(),
            'total_profit' => Deal::closedTrades()
                                  ->dateRange(now()->subDays($days))
                                  ->sum('profit'),
            'avg_trade_profit' => Deal::closedTrades()
                                      ->dateRange(now()->subDays($days))
                                      ->avg('profit'),
    ...
    private function getDailyVolumeTrend($days = 30)
    {
        return Deal::closedTrades()
            ->select(DB::raw('DATE(time) as date'), DB::raw('SUM(volume) as volume'), DB::raw('COUNT(*) as trades'))
            ->dateRange(now()->subDays($days))
            ->groupBy('date')
            ->orderBy('date')
            ->get();
    ...
    private function getTradeDurationStats($days = 30)
    {
        ...
        $closedPositions = Deal::closedTrades()
            ->dateRange(now()->subDays($days))
            ->count();
